...
class janela_principal(CTk):
    def __init__(self):
        ...
        def muda_aba_codigo():
                ...
                def escolha_comecar_codigo(escolha):
                    if escolha == "Nota":
                        ...
                        self.frame_codigos.place_forget()
                        self.frame_anota = CTkFrame(self, width=470, height=360, fg_color=pg)
                        self.frame_anota.pack(pady=10)
                        ...
                        self.botao_voltar_anotas.place(x=392, y=1)
                        ...
                        self.frame_codigos.place_forget()    
                        self.frame_codigo = CTkFrame(self, width=470, height=360, fg_color=pg)
                        self.frame_codigo.pack(pady=10)
                        self.botao_voltar_codigo = CTkButton(self.frame_codigo, text="Voltar", fg_color=cls, width=4, height=22, border_color="black", border_width=2, corner_radius=23, cursor="hand2", command=voltar_code, hover_color=ch)
                        self.botao_voltar_codigo.place(x=404, y=1)
                        self.fonte_tamanho_codigo = CTkOptionMenu(self.frame_codigo, width=90, cursor="hand2", values=["8","10", "12", "14", "16", "18", "20", "22"], fg_color=cls, height=25, corner_radius=15, button_color=cls, text_color=ctb, button_hover_color=ch, dropdown_fg_color=cls, dropdown_text_color=ctb, dropdown_hover_color=ch, command=fonte_tamanho_codigo)
                        self.fonte_tamanho_codigo.place(x=4, y=25)
                    ...
                    if lista_codigo_buscar == True:
                        ...
                        def editar_codigo_busca(evento):
                            ...
                            def voltar_anotacao():
                                ...
                                self.frame_codigos.place(x=20, y=0)
                                frame_anotacao_cb.pack(pady=5, fill="x")

                            id_frame = evento.widget
                            while not hasattr(id_frame, "titulo"):
                                id_frame = id_frame.master
                            titulo_c = id_frame
                            titulo_frame_c = titulo_c.titulo
                            texto_frame_c = titulo_c.texto
                            identidade_frame_c = titulo_c.ID
                            fonte_descricao_frame_c = titulo_c.fonte
                            ...
                            self.frame_edita_c = CTkFrame(self, width=475, height=350, fg_color=pg)
                            self.frame_edita_c.pack(pady=10)
                        ...
                        self.aba_nota = CTkButton(self.frame_guarda_codigos, text="Notas", fg_color=pg, width=4, height=22, cursor="hand2", corner_radius=23, text_color=ct, command=muda_aba_nota_bc)
                        self.aba_nota.place(x=4, y=1) 
                        self.aba_codigo = CTkButton(self.frame_guarda_codigos, text="Códigos", fg_color=pg, width=4, height=22, cursor="hand2", corner_radius=23, text_color=ct, command=muda_aba_codigo)
                        self.aba_codigo.place(x=66, y=1)
                        self.aba_codigo.configure(state=DISABLED)
                        self.botao_voltar_codigos = CTkButton(self.frame_guarda_codigos, text="Voltar", fg_color=cls, width=4, height=22, border_color="black", border_width=2, corner_radius=23, cursor="hand2", command=voltar_code_b, hover_color=ch)
                        ...
                        for lcb in lista_codigo_buscar:
                            ...
                            frame_anotacao_cb.fonte = lcb[3]
                            frame_anotacao_cb.codigo = lcb[4]
                            frame_anotacao_cb.tema = lcb[5]
                            frame_anotacao_cb.linguagem = lcb[6]
                            frame_anotacao_cb.fonte_codigo = lcb[7]
                            ...
                            label_linguagem_cb = CTkLabel(frame_anotacao_cb, text=linguagem_bc.capitalize(), text_color=ct)
                ...
                self.aba_nota.place(x=4, y=1)
                self.aba_codigo = CTkButton(self.frame_guarda_codigos, text="Códigos", fg_color=pg, width=4, height=22, corner_radius=23, command=muda_aba_codigo, text_color=ct)
                self.aba_codigo.place(x=66, y=1)
                self.aba_codigo.configure(state=DISABLED)
        ...
        def buscar(evento):
            materias = ["Português", "Matemática", "História", "Geografia", "Ciências", "Inglês", "Programação", "Outros Estudos"]
